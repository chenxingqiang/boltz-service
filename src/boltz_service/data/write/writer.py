...
from boltz_service.data.types import (
    Interface,
    Record,
    Structure,
)
from boltz_service.data.write.mmcif import to_mmcif
from boltz_service.data.write.pdb import to_pdb


def save_prediction(structure: Structure, path: Path, format: Literal["pdb", "mmcif"] = "mmcif") -> None:
    ...
    path : Path
        The path to save to
    format : str, optional
        The format to save in, by default "mmcif"
    """
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)

    if format == "pdb":
        content = to_pdb(structure)
    else:
        content = to_mmcif(structure)
